...
const getInvoice = async () => {
    console.log("now in get invoice ", props.id)
    let response = await axios.get(`/api/edit_invoice/${props.id}`)
    console.log("Invoices =", response.data.invoice)
...
const Total = () => {
    if(form.value.invoice_items){
        return SubTotal() - ((form.value.discount != null)?form.value.discount:0)
    }else{
        return 0
    }
}
